...
- Topic 08: Azure/PyRIT (github.com/Azure/PyRIT) — archived; the project migrated to github.com/microsoft/PyRIT. Linked to the live microsoft/ repo instead. Azure/ repo has only 13 stars vs. 3.7k at the new location.
- Topic 08: NOVA — seed hint referenced "NOVA" as a tool. Investigated: no canonical open-source tool named NOVA exists for attacking AI systems. The closest match is Novee (novee.security), a commercial beta AI pentesting agent for LLMs (announced Mar 2026, demonstrated at RSA 2026). Novee is not open-source and is still in beta; excluded on both grounds. The seed hint may have been an early working name or a confused reference; defer for future identification.
- Topic 08: LLM Guard (github.com/protectai/llm-guard) — confirmed defensive, not offensive. Sanitizes inputs and scans outputs to protect LLM deployments. Does not attack AI systems. Wrong topic; would belong in a defensive-tooling topic if one existed.
- Topic 08: Adversarial Robustness Toolbox / ART (github.com/Trusted-AI/adversarial-robustness-toolbox) — classical ML adversarial attacks (evasion, poisoning, extraction, inference against classifiers, object detectors, speech models). Not LLM-native. Last release Jul 2025 (v1.20.1); last push Dec 2025 — outside the ~60-day active-maintenance window as of Apr 2026. Excluded: both wrong scope (classical ML vs. generative AI) and stale.
- Topic 08: Giskard (github.com/Giskard-AI/giskard-oss) — v3 is a complete rewrite; the red-team scanner (`giskard-scan`) is marked "in progress" in the v3 roadmap and the v2 scanner is no longer actively maintained. The project's offensive-scanning capability is aspirational in the current version. Excluded from topic 08 on those grounds; revisit when giskard-scan v3 ships.
